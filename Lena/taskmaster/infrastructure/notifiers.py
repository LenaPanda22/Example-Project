...
class SmtpNotifier:
    ...
    def notify(self, recipient: dict, subject: str, body: str) -> bool:
        ...
        if "@" not in to:
            log.error("Ungueltige E-Mail: %r", to)
            return False
        if not subject or not body:
            log.error("Subject und Body duerfen nicht leer sein")
            return False
...
class SmsNotifier:
    def notify(self, recipient: dict, subject: str, body: str) -> bool:
        ...
        if len(number) < 5:
            log.error("Ungueltige Telefonnummer: %r", number)
            return False
...
class PushNotifier:
    def notify(self, recipient: dict, subject: str, body: str) -> bool:
        ...
        if not device:
            log.error("Kein Geraet hinterlegt")
            return False
...
# ---------------------------------------------------------------------------
# COMPOSITE — mehrere Kanaele hinter EINEM Port. Ersetzt "both" und "all".
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DECORATOR — ergaenzt das [DRINGEND]-Praefix. Ersetzt notify_urgent() vollstaendig.
# ---------------------------------------------------------------------------
